...
public class ModelLayer {
	 ...
	 * @param view
	 */
	public void updateLocations(Cell[][] view) {

		//For all the cells in the agens view
		for (int i = 0; i < view[0].length; i++) {
			for (int j = 0; j < view.length; j++) {
				...
				if (c instanceof FuelPump) {
					PositionTuple fpPos = calculatePosition(i, j);
					boolean valid = true;
					...
					StationPosition stationPos = new StationPosition(calculatePosition(i, j), (Station) c);
					boolean valid = true;
					...
					PositionTuple wellPos = calculatePosition(i, j);
					boolean valid = true;
	 ...
	 * @param y
	 */
	public PositionTuple calculatePosition(int x, int y) {

		int tanker_x = DemoSimulator.tank.tankerPos.x;
		int tanker_y = DemoSimulator.tank.tankerPos.y;
	 ...
	 * @return task
	 */
	public TaskPosition findBiggestTaskPlanning(PositionTuple pos) {
		...
		TaskPosition biggest = null;
		int biggestRatio = 0;

		//Find the first uncompleted task in the list
		for (TaskPosition tsk : task) {
			if (tsk.planningCompleted == false && tsk.completed == false) {
				biggest = tsk;
				break;
			}
		}

		//Calculate the biggest per distance travelled
		for (TaskPosition tsk : task) {
			if (tsk.planningCompleted == false && tsk.completed == false) {
				
				//find the distance between the task and the agents position
				int distance = calculateDistance(pos, tsk.pos);
				if(distance == 0) {
					distance = 1;
				}
				//find value per time step
				int currentRatio = tsk.task.getWasteAmount() / distance;
				
				//If its a better value update the best
				if (currentRatio > biggestRatio) {
					biggest = tsk;
	...
	/**
	 * This finds a well thats closest to both the location of the agent and the task for optimal drop offs
	 * @param pos
	 * @param target
	 * @return well position
	 */
	public PositionTuple findClosestWell(PositionTuple pos, PositionTuple target) {
		...
		if (well.isEmpty()) { // If there are no fuel pumps in location
			...
			//For all the wells discovered
			for (PositionTuple i : well) {
				PositionTuple well_coord = new PositionTuple(pos.x - i.x, pos.y - i.y);
				int wellDist = Math.max(Math.abs(well_coord.x), Math.abs(well_coord.y));
				int targetDist = calculateDistance(i, target);

				//Find the well of least distance
				if (dist > wellDist + targetDist) {
					closest = i;
					dist = wellDist + targetDist;
				}
			}
	 ...
	 * @return
	 */
	public int locationDirection(PositionTuple location) {

		int dPos = 0;
		int x, y;
		x = DemoSimulator.tank.tankerPos.x - location.x;
		y = DemoSimulator.tank.tankerPos.y - location.y;
